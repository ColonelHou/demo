package com.atguigu.apitest.pvanduv

import com.atguigu.apitest.util.UserBehavior
...
//    val stream = env.socketTextStream("localhost", 9999, '\n')
  ...
  class UvTrigger extends Trigger[(String, Long), TimeWindow] {
    // 每来一条数据都会触发一次ProcessWindowFunction函数的执行, 并清空所有窗口状态
    ...
    override def onEventTime(time: Long, window: TimeWindow, ctx: Trigger.TriggerContext): TriggerResult = {
      if (time == window.getEnd) {
        ...
        TriggerResult.FIRE_AND_PURGE // 触发计算并且完成清除
      }
      TriggerResult.CONTINUE // 继续处理当前窗口
    }
  ...
  class UvWindowProcess extends ProcessWindowFunction[(String, Long), String, String, TimeWindow] {
    ...
    // element其实是所有数据, 因为上面定义的出发器, 所以每次拿到的是一条数据
